// ==========================================
// ask-chat.js — UPDATED WITH GROQ PROVIDER
// ==========================================
(function () {
  ...
  // ---- INITIALIZATION ----
  updateNotesCount();
  addBotMsg("Hi 👋 — Main aapka Study Assistant hu! Aap koi bhi study question pooch sakte hain ya commands use kar sakte hain (e.g., 'open pdf 9 chemistry ch3', 'formula class 10', 'calendar').");
  ...
  async function handleCommand(raw) {
      ...
      addBotMsg(`
        <div style="border: 1px solid #38bdf8; padding: 10px; border-radius: 8px; background: rgba(56, 189, 248, 0.08); margin-top: 5px;">
          ...
          <div style="font-size: 12px; color: #e0e7ff; line-height: 1.5; margin-bottom: 8px;">
            Is application ko <b>Rohit</b> ne banaya hai! Unke aur is platform ke bare me aur janne ke liye niche diye gaye link par click karein:
          </div>
    ...
    // DEFAULT: GROQ AI AGENT CALL (Fast & High Limit)
    await askGroqAI(raw);
  }
  ...
  async function fetchAndSuggestFormulas(queryText) {
    ...
    if (formulaData && formulaData.length > 0) {
      ...
      formulaData.forEach(f => {
        ...
        if (f.type === 'text') html += `<div style="color: #60a5fa; font-weight: 600;">📝 ${f.formula_text}</div>`;
        html += `</div>`;
      });
    } else {
...
You are a study assistant for this learning platform (roitx.github.io). Answer in simple language.
When asked for pages or links, generate HTML standard <a> tags using the relative paths below.
...
- <a href="ask-doubt.html">Ask Doubt Page</a>
- <a href="solver.html">Doubt Solver</a>
`;
  // ---- GROQ AI INTEGRATION (FIXED SYSTEM KNOWLEDGE) ----
  async function askGroqAI(userQuery) {
    ...
    // System Knowledge aur Rules ko User Context ke sath clearly format kiya gaya hai
    const fullPrompt = `IMPORTANT INSTRUCTIONS & SYSTEM KNOWLEDGE:
${systemKnowledge}

USER LANGUAGE PREFERENCE: ${selectedLanguage === 'en' ? 'English' : 'Hindi / Hinglish'}
...
${userQuery}`;
    ...
    try {
      const response = await fetch(SUPABASE_FUNCTION_URL, {
        ...
        body: JSON.stringify({ 
          prompt: fullPrompt, // 🚀 Combined Prompt Bheja Ja Raha Hai
