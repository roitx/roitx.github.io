...
window.addEventListener("deviceorientation", (e) => {
    if (controlMode === "tilt" && game && !paused) {
        let tilt = e.gamma; 
        if (tilt > 5 && player.x + player.w < canvas.width) player.x += 5;
        if (tilt < -5 && player.x > 0) player.x -= 5;
    }
});
...
function spawnMonster() {
    ...
    // Monster starting position above the screen
    monster = { x: canvas.width / 2 - 60, y: -120, w: 120, h: 90, hp: 30, speed: 2, dir: 1 };
}

function shoot() {
    if (player.cooldown > 0) return;
    bullets.push({ x: player.x + player.w / 2 - 2, y: player.y });
    player.cooldown = 15; // Fast auto-shoot
}

function spawnEnemy() {
    if (monsterActive) return; // Boss ke time chhote dushman nahi aayenge
    const s = 35;
    enemies.push({ x: Math.random() * (canvas.width - s), y: -s, w: s, h: s, speed: 2 + level * 0.3 });
}

function explode(x, y, color) {
    for (let i = 0; i < 12; i++)
        particles.push({ x, y, vx: (Math.random() - 0.5) * 6, vy: (Math.random() - 0.5) * 6, l: 20, c: color });
}
...
function update() {
    ...
    if (monsterActive && monster) {
        // Move into view then side to side
        if (monster.y < 80) monster.y += 2;
            ...
            if (Math.random() < 0.04) enemyBullets.push({ x: monster.x + monster.w/2, y: monster.y + monster.h, speed: 5 });
        }
        ...
        bullets.forEach((b, bi) => {
            if (b.x > monster.x && b.x < monster.x + monster.w && b.y < monster.y + monster.h && b.y > monster.y) {
                ...
                if (monster.hp <= 0) {
                    ...
                    monsterActive = false;
                    monster = null;
                    explode(monster.x + monster.w/2, monster.y + monster.h/2, "orange");
                }
            }
        });
    }
    ...
    enemies.forEach((e, i) => {
        ...
        bullets.forEach((b, j) => {
            if (b.x < e.x + e.w && b.x + 4 > e.x && b.y < e.y + e.h && b.y + 15 > e.y) {
                ...
                score += 10; updateHUD();
                if (score > 0 && score % 200 === 0) spawnMonster();
...
function startGame() {
    ...
    score = 0; lives = 10; level = 1; monsterActive = false; monster = null;
