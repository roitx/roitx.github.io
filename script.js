...
function showMotivation() {
  ...
  setTimeout(() => modal.classList.remove('modal-active'), 5000); // Close modal after 5 seconds
}
...
let dots = [];
const numDots = Math.floor(window.innerWidth / 30); // Adjust the number of dots based on screen width
...
function createDots() {
  ...
  for (let i = 0; i < numDots; i++) {
    dots.push({
      ...
      r: 1.5 + Math.random() * 2, // Random radius
      dx: (Math.random() - 0.5) * 0.4, // Random horizontal speed
      dy: (Math.random() - 0.5) * 0.4 // Random vertical speed
    });
  }
}

// Resize canvas and reposition dots on window resize
function resizeCanvas() {
  c.width = window.innerWidth;
  c.height = window.innerHeight;
  createDots(); // Recreate dots for the new size
}

// Initialize canvas and resize on window resize
resizeCanvas();
window.addEventListener("resize", resizeCanvas);

// Animate dots to move across the canvas
function animateDots() {
  ctx.clearRect(0, 0, c.width, c.height); // Clear the canvas on each frame
  dots.forEach(d => {
    ctx.beginPath();
    ctx.arc(d.x, d.y, d.r, 0, Math.PI * 2); // Draw circle (dot)
    ctx.fillStyle = "rgba(255,255,255,0.25)";
    ctx.fill();
    d.x += d.dx; // Update dot's horizontal position
    d.y += d.dy; // Update dot's vertical position
    if (d.x < 0 || d.x > c.width) d.dx *= -1; // Reverse direction when hitting edge
    if (d.y < 0 || d.y > c.height) d.dy *= -1; // Reverse direction when hitting edge
  });
  requestAnimationFrame(animateDots); // Request next animation frame
}
